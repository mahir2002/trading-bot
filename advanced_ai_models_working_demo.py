#!/usr/bin/env python3
"""
Advanced AI Models Working Demo
Comprehensive demonstration of LSTM, ensemble methods, and sophisticated features
"""
...
class AdvancedFeatureEngineer:
    ...
    def _add_volatility_features(self, df):
        ...
        # Volatility regime
        vol_median = df['volatility_20'].rolling(window=50, min_periods=20).median()
    ...
    def _add_onchain_features(self, df):
...
class LSTMModel:
    ...
    def __init__(self, sequence_length=60, units=[128, 64], dropout=0.2):
    ...
    def prepare_sequences(self, data):
        ...
        for i in range(self.sequence_length, len(data)):
            ...
            y.append(data[i, 0])  # Predict first feature (returns)
    ...
    def predict(self, X_test):
        ...
        if 'lstm' in self.models:
            ...
            if len(X_lstm) > 0:
                ...
                full_pred = np.full(len(X_test), lstm_pred[0] if len(lstm_pred) > 0 else 0)
                if len(lstm_pred) > 0:
...
def generate_realistic_market_data(n_samples=2000):
...
def demonstrate_advanced_ai_models():
    ...
    features_df = feature_engineer.create_comprehensive_features(market_data)
    ...
    print(f"   Feature categories: Technical, Lagged, Volatility, Sentiment, On-chain, Temporal")
    
    # Train advanced ensemble
    print(f"\n🤖 Training Advanced Ensemble Models...")
    ensemble = AdvancedEnsemble(sequence_length=min(60, len(X_train)//4))
    ...
    feature_categories = {
        'Technical': [f for f in numeric_columns if any(x in f for x in ['sma', 'ema', 'rsi', 'macd', 'bb', 'stoch', 'williams', 'atr', 'obv', 'mfi'])],
        'Lagged': [f for f in numeric_columns if 'lag' in f],
        'Volatility': [f for f in numeric_columns if any(x in f for x in ['volatility', 'vol', 'parkinson', 'gk'])],
        'Sentiment': [f for f in numeric_columns if any(x in f for x in ['sentiment', 'fear', 'greed'])],
        'On-chain': [f for f in numeric_columns if any(x in f for x in ['active', 'transaction', 'exchange', 'whale', 'hash', 'difficulty', 'open_interest', 'funding'])],
        'Statistical': [f for f in numeric_columns if any(x in f for x in ['mean', 'std', 'skew', 'kurt', 'position', 'momentum', 'reversion'])],
        'Temporal': [f for f in numeric_columns if any(x in f for x in ['hour', 'day', 'month', 'weekend', 'end'])],
...
def create_comprehensive_visualization(results):
    ...
    ax3.scatter(ensemble_pred, residuals, alpha=0.6, s=20)
    ...
    if results['feature_importance'] is not None:
        ...
        ax5.set_yticklabels([f[:20] for f in top_features['feature']], fontsize=8)
        ax5.set_xlabel('Importance')
        ax5.set_title('Top 12 Feature Importance')
    ...
    bars = ax6.bar(metrics, values, color=['blue', 'green', 'red', 'orange'])
    ax6.set_ylabel('Value')
    ax6.set_title('Ensemble Performance Metrics')
    ...
    ax7 = axes[2, 0]
    ax7.hist(actual, bins=40, alpha=0.7, label='Actual', density=True, color='blue')
    ax7.hist(ensemble_pred, bins=40, alpha=0.7, label='Predicted', density=True, color='red')
    ax7.set_xlabel('Returns')
    ax7.set_ylabel('Density')
    ax7.set_title('Returns Distribution Comparison')
    ax7.legend()
    ...
    if results['feature_importance'] is not None and results['feature_categories']:
        ...
        if category_importance:
            ...
            bars = ax8.bar(categories, importances)
            ax8.set_ylabel('Total Importance')
            ax8.set_title('Feature Category Importance')
    ...
    for i in range(window_size, len(actual)):
        ...
        window_pred = ensemble_pred[i-window_size:i]
        window_acc = np.mean(np.sign(window_pred) == np.sign(window_actual))
        rolling_accuracy.append(window_acc)
    
    if rolling_accuracy:
        ax9.plot(range(window_size, len(actual)), rolling_accuracy, linewidth=2)
        ax9.axhline(y=0.5, color='r', linestyle='--', alpha=0.7, label='Random')
        ax9.set_xlabel('Time')
        ax9.set_ylabel('Rolling Accuracy')
        ax9.set_title(f'Rolling Directional Accuracy ({window_size}-period)')
        ax9.legend()
