...
class MarketRegimeDetector:
    ...
    def detect_regime(df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_volatility = df['volatility'].rolling(50).mean().iloc[-1] if 'volatility' in df.columns else 0
            ...
            return {
                ...
                'momentum': (current_price - df['close'].iloc[-20]) / df['close'].iloc[-20] if len(df) >= 20 else 0
            }
    ...
    def calculate_entry_signals(self, df: pd.DataFrame) -> Optional[TradingSignal]:
        ...
        try:
            ...
            signal = TradingSignal(
                symbol=df.get('symbol', ['UNKNOWN'])[0] if hasattr(df.get('symbol', 'UNKNOWN'), '__iter__') else 'UNKNOWN',
...
class MomentumStrategy(TradingStrategy):
    ...
    def calculate_entry_signals(self, df: pd.DataFrame) -> Optional[TradingSignal]:
        ...
        try:
            ...
            sma_20 = df_with_indicators['sma_20'].iloc[-1] if 'sma_20' in df_with_indicators.columns else price
            volume_ratio = df_with_indicators['volume_ratio'].iloc[-1] if 'volume_ratio' in df_with_indicators.columns else 1.0
                ...
                for strategy_name, strategy in self.strategies.items():
                    try:
                        ...
                        if signal and signal.action != 'HOLD':
                            ...
                            self.logger.info(f"📊 {strategy_name} signal: {signal.action} {symbol} (confidence: {signal.confidence:.2f})")
                    except Exception as e:
                        self.logger.error(f"Signal generation failed for {strategy_name} on {symbol}: {e}")
    ...
    def store_signal(self, signal: TradingSignal):
        ...
        try:
            ...
            """, (
                ...
                signal.price_target, signal.stop_loss, signal.take_profit,
                signal.strategy_name, signal.expected_return, signal.risk_level
            ))
...
async def main():
    ...
    while cycle_count < 5:  # Run 5 cycles for demo
        try:
            ...
            if result:
                print(f"🔄 Trading cycle {cycle_count + 1}: {result['executed_signals']} signals executed")
