...
class EnsembleAgent(BaseAgent):
    ...
    def __init__(self, agents: List[BaseAgent], config: Optional[EnsembleConfig] = None):
    ...
    async def analyze(self, data: Dict[str, Any]) -> AgentSignal:
        ...
        if not self.agents:
            ...
            return AgentSignal(SignalAction.HOLD, 0.5, {"error": "no_agents"}, "ensemble")
        ...
        tasks = [self._analyze_with_timeout(agent, data) for agent in self.agents]
        ...
        metadata = {
            "individual_signals": {name: sig.to_dict() for name, sig in valid_signals},
    ...
    async def _analyze_with_timeout(self, agent: BaseAgent, data: Dict[str, Any]) -> AgentSignal:
        """Analyze with timeout to prevent hanging."""
        try:
            return await asyncio.wait_for(agent.analyze(data), timeout=5.0)  # 5 second timeout
        except asyncio.TimeoutError:
            raise TimeoutError(f"Agent {agent.name} timed out")

    def _weighted_vote(self, signals: List[Tuple[str, AgentSignal]]) -> Dict[str, Any]:
        """
    ...
    def _calculate_consensus(self, signals: List[Tuple[str, AgentSignal]]) -> float:
    ...
    def _calibrate_confidence(self, raw_confidence: float, consensus: float) -> float:
        """
        ...
        calibrated = calibration_factor * raw_confidence + (1 - calibration_factor) * consensus
    ...
    async def _update_weights_from_performance(self) -> None:
        ...
        try:
            ...
            for agent in self.agents:
                accuracy = agent.get_confidence_weighted_accuracy(self.config.accuracy_window)
