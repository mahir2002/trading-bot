...
class BaseAgent(ABC):
    ...
    def record_performance(self, prediction: AgentSignal, actual_return: float) -> None:
        ...
        correct = (
            (prediction.is_buy() and actual_return > 0)
            or (prediction.is_sell() and actual_return < 0)
            or (prediction.action == SignalAction.HOLD and abs(actual_return) < 0.01)
        )
    ...
    def get_confidence_weighted_accuracy(self, window: int = 100) -> float:
        ...
        recent = self._performance_history[-window:]
        weighted_correct = sum(r["confidence"] if r["correct"] else 0 for r in recent)
